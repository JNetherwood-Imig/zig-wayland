...
pub fn main() !void {
    ...
    const output_file = try std.fs.cwd().createFile(ctx.output_path, .{});
    defer output_file.close();
    var output_buffer: [8192]u8 = @splat(0);
...
const Context = struct {
    ...
    pub fn init(gpa: Allocator) !Context {
        ...
        while (args.next()) |arg| {
            if (std.mem.eql(u8, arg, "-m")) {
                ...
                if (std.mem.eql(u8, mode_str, "client"))
                    mode = .client
                else if (std.mem.eql(u8, mode_str, "server"))
                    mode = .server
                else
                ...
                defer gpa.free(data);
                const p = try xml.parse(xml.Document(RawProtocol), gpa, data);
                errdefer p.deinit(gpa);
                try raw_protocols.append(gpa, p);
                for (p.value.interfaces.value.items) |i| {
                    const entry = InterfaceMapEntry{
                        .protocol = try gpa.dupe(u8, p.value.name.value),
                        .type_name = try typeName(gpa, prefix, i.name.value),
                    };
        ...
        for (raw_protocols.items) |p| {
            const protocol = try Protocol.fromRaw(gpa, interface_map, p.value);
            errdefer protocol.deinit(gpa);
            try protocols.append(gpa, protocol);
        }
    ...
    pub fn deinit(self: *Context, gpa: Allocator) void {
        ...
        while (it.next()) |e| {
            gpa.free(e.key_ptr.*);
            gpa.free(e.value_ptr.protocol);
            gpa.free(e.value_ptr.type_name);
        }
    ...
    const Protocol = struct {
        ...
        pub fn fromRaw(gpa: Allocator, map: InterfaceMap, raw: RawProtocol) !Protocol {
            ...
            const description = if (raw.description.value) |desc|
                try gpa.dupe(u8, desc.body.data)
            else
                null;
...
fn writeClientInterface(
    ...
    const name = typeName(gpa, prefix, interface.name.value);
    defer gpa.free(name);
...
fn typeName(gpa: Allocator, prefix: []const u8, name: []const u8) ![]u8 {
    const stripped = stripped: {
        if (!std.mem.startsWith(u8, name, prefix)) {
            std.log.err("Prefix {s} not found in name {s}\n", .{ prefix, name });
            return error.PrefixNotFound;
        }
        const prefix_stripped = name[prefix.len..];
        ...
        break :stripped version_stripped;
    };
    return snakeToPascal(gpa, stripped);
}

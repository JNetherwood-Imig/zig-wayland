...
pub fn serializeArgs(
    ...
    switch (@typeInfo(@TypeOf(args))) {
        .@"struct" => |s| inline for (s.fields) |f| {
            index += try serializeArg(buffer[index..], @field(args, f.name));
        },
...
fn serializeInt(buffer: []u8, int: i32) !void {
    std.mem.bytesAsValue(i32, buffer[0..@sizeOf(int)]).* = int;
    return @sizeOf(i32);
}
...
fn serializeFixed(buffer: []u8, fixed: Fixed) !void {
    return serializeInt(buffer, fixed.data);
}

fn serializeString(buffer: []u8, string: ?String) !void {
    if (string) |s| {
...
fn serializeGenericNewId(buffer: []u8, new_id: GenericNewId) !void {
...
fn serializeArray(buffer: []u8, array: Array) !void {

...
pub fn serializeMessage(buf: []u8, object: u32, opcode: u16, args: anytype) SerializeError!usize {
    ...
    if (length > libwayland_max_message_size) return error.MessageTooLong;
...
/// Returns `error.InvalidData` if `bytes` or `fds` is found to contain invalid data
/// for constructing `T`.
    ...
    switch (@typeInfo(T)) {
        .@"struct" => |s| inline for (s.fields[1..], signature) |field, sig_byte| {
            if (sig_byte == 'd') { // field is an fd
                if (fd_index == fds.len) return error.InvalidData;
                @field(event, field.name) = fds[fd_index];
                fd_index += 1;
                continue;
            }

            if (index >= bytes.len) return error.InvalidData;
...
fn deserializeArray(data: []const u8) DeserializeError!struct { []const u8, usize } {
    ...
    if (roundup4(len) > data.len - 4) return error.InvalidData;
...
fn deserializeString(data: []const u8) DeserializeError!struct { [:0]const u8, usize } {
    if (data.len < 4) return error.InvalidData;
    const len, _ = try deserializeUint(data);
    if (len == 0 or roundup4(len) > data.len - 4) return error.InvalidData;
...
fn deserializeGenericNewId(data: []const u8) DeserializeError!struct { GenericNewId, usize } {
    ...
    if (len + 8 > data.len) return error.InvalidData;
    const version, _ = try deserializeUint(data[len..]);
    const new_id, _ = try deserializeUint(data[len..][4..]);

    if (version == 0 or new_id == 0) return error.InvalidData;
...
fn deserializeEnum(comptime T: type, data: []const u8) DeserializeError!struct { T, usize } {
    ...
    const enum_val = std.enums.fromInt(T, val) orelse return error.InvalidData;
    return .{ enum_val, len };
}
